...
local view = require("gh.issues.view")
...
---@param _bufnr integer Buffer number (unused, kept for API consistency)
local function setup_issue_list_highlights(_bufnr)
...
---@return table<integer, {title: string|nil, state: string|nil}>|nil Changes keyed by issue number,
...
function M.open_issue_list(repo, opts)
  ...
  if append then
    ...
    vim.notify(
      string.format("Loading issues %d-%d...", currently_loaded + 1, new_limit),
      vim.log.levels.INFO
    )
    ...
    cli.issue.list(cli_opts, function(success, all_issues, error)
      if not success then
        vim.notify(
          "Failed to fetch more issues: " .. (error or "unknown error"),
      ...
      if not vim.api.nvim_buf_get_var(bufnr, "gh_loading") then
        vim.notify(
          string.format("Loaded %d more issues (%d total)", added, #all_issues),
          vim.log.levels.INFO
        )
      end
    end)
  ...
  cache.get_or_fetch(
    cache_key,
    function(callback)
      cli.issue.list({ limit = limit, state = state, repo = repo }, function(success, issues, error)
        if success then
          callback(issues)
        else
    ...
    function(issues)
      ...
      buffer.on_write(bufnr, function(buf)
        ...
        local changes, validation_error =
          parse_issue_list_changes(current_lines, original_collection)
        ...
        for number, change in pairs(changes) do
          ...
          if change.title then
            cli.issue.edit(number, { title = change.title }, target_repo, function(success, error)
              pending = pending - 1
              if not success then
        ...
        ["<CR>"] = {
          callback = function()
            ...
            if number then
              ...
              view.open_issue_detail(tonumber(number), repo)
            end
          end,
      ...
      -- Add delete keymap if enabled
      local config = require("gh.config")
      if config.opts.issue_list.enable_delete then
        local delete = require("gh.issues.delete")
        buffer.set_keymaps(bufnr, {
          ["dd"] = {
            callback = function()
              delete.delete_issue_at_cursor(bufnr, repo)
            end,
            desc = "Delete issue",
          },
        })
      ...
      -- Set up autocmd to refresh on issue create/delete
      local group = vim.api.nvim_create_augroup("GhIssueListRefresh_" .. bufnr, { clear = true })
      vim.api.nvim_create_autocmd("User", {
        group = group,
        pattern = { "GhIssueCreated", "GhIssueDeleted", "GhIssueUpdated" },
        callback = function(event)
          -- Only refresh if this buffer is still valid and matches the repo
            ...
            cli.issue.list({ state = buf_state, repo = repo }, function(success, fresh_issues)
              if success then
                vim.schedule(function()
                  ...
                  local fresh_collection = IssueCollection.new(fresh_issues or {})
                  local fresh_filter_context = { state = buf_state }
                  local fresh_lines = format_issue_list(fresh_collection, fresh_filter_context)
                  buffer.set_lines(bufnr, fresh_lines)
                  vim.api.nvim_buf_set_var(
                    bufnr,
                    "gh_issues_collection",
...
function M.setup_autocmds()
  ...
  vim.api.nvim_create_autocmd({ "BufReadCmd", "BufNewFile" }, {
    group = group,
    pattern = { "gh://issues", "gh://issues/*" },
    callback = function(args)
      local bufnr = args.buf
      vim.bo[bufnr].buftype = "acwrite"
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "Loading issues from GitHub..." })
  ...
  vim.api.nvim_create_autocmd({ "BufReadCmd", "BufNewFile" }, {
    group = group,
    pattern = { "gh://issue/*", "gh://issue/*/*", "gh://issue/*/*/*" },
    callback = function(args)
      ...
      if number then
        ...
        vim.bo[bufnr].buftype = "acwrite"
        vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "Loading issue detail from GitHub..." })

        -- Fetch and refresh the issue in the current buffer
        cli.issue.view(tonumber(number), repo, function(success, issue, error)
          if not success then
            vim.notify(
              "Failed to fetch issue: " .. (error or "unknown error"),
              vim.log.levels.ERROR
            )
            return
          end
          ...
          local lines = view.format_issue_detail(issue)
          buffer.set_lines(bufnr, lines)
          ...
          view.add_issue_metadata_virtual_text(bufnr)

          -- Set up write handler if not already set
          buffer.on_write(bufnr, function(buf)
            local current_lines = buffer.get_lines(buf)
            local parsed = view.parse_issue_detail(current_lines)
  ...
  vim.api.nvim_create_autocmd({ "BufReadCmd" }, {
    ...
    callback = function(args)
      ...
      -- Open new issue buffer (which will create a new buffer)
      local create = require("gh.issues.create")
      create.create_issue_buffer({ repo = repo })

      -- After create_issue_buffer creates the new buffer, delete this placeholder
      -- Wait a bit longer to ensure the new buffer is fully created and switched to
      vim.defer_fn(function()
        if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_get_current_buf() ~= bufnr then
          -- Only delete if the buffer name is still the placeholder
          local buf_name = vim.api.nvim_buf_get_name(bufnr)
          if buf_name == args.file then
...
M._format_issue_list = format_issue_list
