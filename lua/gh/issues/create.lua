...
---@param opts table Options: { repo: string|nil, template: string|nil, title: string|nil, assignees: string[]|nil, labels: string[]|nil, milestone: string|nil, project: string|nil }
function M.create_issue_buffer(opts)
  ...
  local function create_buffer_with_content(template_content)
    ...
    -- Create buffer
    local buf_name = repo and ("gh://issue/new/" .. repo) or "gh://issue/new"
        ...
        cli.create_issue(create_opts, function(success, issue, error)
          ...
          vim.schedule(function()
            ...
            if issue.number then
              ...
              local new_name = target_repo and 
                string.format("gh://issue/%s/%d", target_repo, issue.number) or 
                string.format("gh://issue/%d", issue.number)
              vim.api.nvim_buf_set_name(buf, new_name)
              
              -- Fetch the full issue data to update virtual text with actual data
              cli.get_issue(issue.number, target_repo, function(fetch_success, full_issue, fetch_error)
                if fetch_success and full_issue then
                  vim.schedule(function()
                    ...
                    local render = require("gh.ui.render")
                    render.render_metadata(buf, full_issue)
                  end)
                end
              end)
            else
  ...
  if opts.template then
    ...
    cli.list_issue_templates(repo, function(success, templates, error)
      ...
      vim.ui.select(template_names, {
        ...
        if selected_template then
          cli.get_issue_template(repo, selected_template.path, function(success, content, error)
            if not success then
              vim.notify("Failed to fetch template: " .. (error or "unknown error"), vim.log.levels.WARN)
              create_buffer_with_content("")
              return
...
function M.get_assignee_completions(repo, callback)
  ...
  cache.get_or_fetch(
    ...
    function(cb)
      ...
      cli.list_contributors(repo, function(success, contributors, error)
...
function M.get_label_completions(repo, callback)
  ...
  cache.get_or_fetch(
    cache_key,
    function(cb)
      cli.list_labels(repo, function(success, labels, error)
...
function M.get_milestone_completions(repo, callback)
  ...
  cache.get_or_fetch(
    cache_key,
    function(cb)
      cli.list_milestones(repo, function(success, milestones, error)
...
function M.get_project_completions(repo, callback)
  ...
  cache.get_or_fetch(
    cache_key,
    function(cb)
      cli.list_projects(repo, function(success, projects, error)
