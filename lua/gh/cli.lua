...
function M.list_issue_templates(repo, callback)
  ...
  local args = { "api", "repos/{owner}/{repo}/contents/.github/ISSUE_TEMPLATE", "--jq", ".[] | select(.type == \"file\") | {name: .name, path: .path}" }
  if repo then
    table.insert(args, "--repo")
    table.insert(args, repo)
  end
...
--- Create a new issue
---@param opts table Options: { title: string, body: string, assignees: string[], labels: string[], milestone: string, project: string, repo: string|nil }
---@param callback fun(success: boolean, issue: table|nil, error: string|nil)
