...
---@return table|nil Filter context with keys: state, assignee, author, label, mention, milestone, search, or nil if validation failed
...
function M.apply_filters(bufnr)
  ...
  local ok, repo = pcall(vim.api.nvim_buf_get_var, bufnr, "gh_repo")
  if not ok then
    repo = nil
  end
  if repo == "" then
...
function M.update_filter_display(bufnr)
  ...
  if ok and collection_data then
    ...
    for i, issue in collection:iter() do
      if issue_line_num < total_lines then
