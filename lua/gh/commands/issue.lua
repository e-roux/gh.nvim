...
function M.complete(arg_lead, args)
  ...
  if subcommand == "create" then
    ...
    if prev_arg == "--assignee" or prev_arg == "-a" then
      ...
      issues.get_assignee_completions(repo, function(assignees)
    ...
    if prev_arg == "--label" or prev_arg == "-l" then
      local issues = require("gh.issues")
      issues.get_label_completions(repo, function(labels)
        -- Cache for next completion attempt
      end)
      return {}
    end
    ...
    if prev_arg == "--milestone" or prev_arg == "-m" then
      local issues = require("gh.issues")
      issues.get_milestone_completions(repo, function(milestones)
        -- Cache for next completion attempt
      end)
      return {}
    end
    ...
    if prev_arg == "--project" or prev_arg == "-p" then
      local issues = require("gh.issues")
      issues.get_project_completions(repo, function(projects)
        -- Cache for next completion attempt
      end)
      return {}
    end
    ...
    if prev_arg == "--template" then
      local issues = require("gh.issues")
      issues.get_template_completions(repo, function(templates)
        -- Cache for next completion attempt
      end)
      return {}
    end
