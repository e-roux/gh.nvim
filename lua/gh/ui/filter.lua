...
--- Uses simple extmark-based filter_inputs system
local M = {}
...
--- Extmark IDs for tracking input positions
M.extmark_ids = {}
...
M.FIRST_ISSUE_LINE = #filter_inputs.inputs + 2 -- +1 for header, +1 for 1-indexed
...
function M.parse_filter_context(bufnr)
  -- Use filter_inputs to get all values
  local values = filter_inputs.get_values(bufnr, M.namespace, M.extmark_ids)
  local context = {}
  local errors = {}
    ...
    if value ~= "" then
      ...
      elseif key == "state" then
        ...
        if state_lower ~= "open" and state_lower ~= "closed" and state_lower ~= "all" then
          table.insert(
            errors,
            string.format("State must be 'open', 'closed', or 'all' (got '%s')", value)
          )
        else
          context[key] = state_lower
        end
      else
        context[key] = value
      end
...
function M.update_filter_display(bufnr)
  ...
  for i, input in ipairs(filter_inputs.inputs) do
    -- Add filter name in normal color (strip emoji)
    local filter_name = input.label:match("^[%s%p]*(.+)$") or input.label
    filter_name = filter_name:gsub("^%s+", ""):gsub("%s+$", "")
    table.insert(help_virt_text, { filter_name, "Normal" })
    table.insert(help_virt_text, { " ", "Normal" })
    table.insert(help_virt_text, { localleader .. input.keymap, "Cyan" })

    if i < #filter_inputs.inputs then
      table.insert(help_virt_text, { "  ", "Normal" })
    end
  end
  ...
  -- Render all filter inputs using filter_inputs module
  filter_inputs.render(bufnr, M.namespace, M.extmark_ids)
...
function M.setup_filter_keymaps(bufnr)
  -- Use filter_inputs.setup_keymaps for simple keymap setup
  filter_inputs.setup_keymaps(bufnr, M.namespace, M.extmark_ids)
end
...
function M.setup_auto_filter(bufnr)
  ...
  local function get_filter_state()
    ...
    for _, input in ipairs(filter_inputs.inputs) do
      table.insert(state_parts, values[input.name] or "")
    end
  ...
  local function position_cursor()
    ...
    local is_filter_line = false
    for _, input in ipairs(filter_inputs.inputs) do
      if input.line == line_num then
        is_filter_line = true
        break
      end
    end
  ...
  vim.api.nvim_create_autocmd({ "TextChanged", "TextChangedI" }, {
    ...
    callback = function()
      ...
      -- Update filter display (refresh placeholders)
      filter_inputs.update_display(bufnr, M.namespace, M.extmark_ids)
      ...
      -- Only process if cursor is on one of the filter lines
      if line_num >= 1 and line_num < M.FIRST_ISSUE_LINE then
  ...
  vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
    ...
    callback = function()
      ...
      if line_num == 1 then
        -- Move to first filter line at column 0
        local first_input = filter_inputs.inputs[1]
        vim.cmd(
          string.format("noautocmd call nvim_win_set_cursor(0, [%d, %d])", first_input.line, 0)
        )
        return
      end

      -- Don't force cursor positioning in normal mode
      -- Let Neovim handle the visual positioning naturally
      -- The virtual text overlay will make it appear in the right place
  ...
  vim.schedule(function()
    ...
    local first_input = filter_inputs.inputs[1]
    vim.api.nvim_win_set_cursor(0, { first_input.line, 0 })
