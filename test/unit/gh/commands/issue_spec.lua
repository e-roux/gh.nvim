...
describe("commands.issue", function()
  ...
  describe("completion", function()
    ...
    it("should complete close flags", function()
      local completions = issue_cmd.complete("--", { "close", "--" })
      assert.is_true(vim.tbl_contains(completions, "--repo"))
      assert.is_true(vim.tbl_contains(completions, "--comment"))
      assert.is_true(vim.tbl_contains(completions, "--reason"))
    end)

    it("should complete reopen flags", function()
      local completions = issue_cmd.complete("--", { "reopen", "--" })
      assert.is_true(vim.tbl_contains(completions, "--repo"))
      assert.is_true(vim.tbl_contains(completions, "--comment"))
      assert.is_false(vim.tbl_contains(completions, "--reason"))
    end)

    it("should complete reason values for close", function()
      local completions = issue_cmd.complete("", { "close", "--reason" })
      assert.is_true(vim.tbl_contains(completions, "completed"))
      assert.is_true(vim.tbl_contains(completions, "not planned"))
    end)
  end)
end)
